...
public class FillPumpOpSchedPanel extends JPanel
{
	...
	// Constructor
	public FillPumpOpSchedPanel(JFrame currentFrame, Pool pool)
	{
		this.setLayout(null);
	...
	public void checkBox(Pool pool, JFrame currentFrame)
	{
		...
		{    
            ...
            {                 
            	...
            	{
            		pool.getFillPump().setDT(true);
            		
            	}
            	else
            	{
            		pool.getFillPump().setDT(false);
            	}
            }    
         });  
	}
	...
	public void enter(Pool pool, JFrame currentFrame)
	{
		...
		btnNewButton_2.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e) 
			{ 
				pool.getFillPump().setOP(true);
				pool.getFillPump().setInitiate(pool.getFillPump().stringToLocalDateTime(textField1.getText()));
				pool.getFillPump().setTerminate(pool.getFillPump().stringToLocalDateTime(textField2.getText()));
				pool.setPanelIndex(2);
				currentFrame.getContentPane().removeAll();
				currentFrame.getContentPane().add(new FillPumpModPanel(currentFrame, pool));
				currentFrame.getContentPane().revalidate();
