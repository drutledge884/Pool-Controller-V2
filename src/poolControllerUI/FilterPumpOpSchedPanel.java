...
public class FilterPumpOpSchedPanel extends JPanel
{
	...
	// Constructor
	public FilterPumpOpSchedPanel(JFrame currentFrame, Pool pool)
	{
		this.setLayout(null);
	...
	public void checkBox(Pool pool, JFrame currentFrame)
	{
		...
		{    
            ...
            {                 
            	...
            	{
            		pool.getFilterPump().setDT(true);
            		
            	}
            	else
            	{
            		pool.getFilterPump().setDT(false);
            	}
            }    
         });  
	}
	...
	public void back(Pool pool, JFrame currentFrame)
	{
		...
		btnNewButton_2.addActionListener(new ActionListener()
		{
			...
			{ 
				...
				System.out.println("To Mod From Sched");
			}
		});
	...
	public void enter(Pool pool, JFrame currentFrame)
	{
		...
		btnNewButton_2.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e) 
			{ 
				pool.getFilterPump().setOP(true);
				pool.getFilterPump().setInitiate(pool.getFilterPump().stringToLocalDateTime(textField1.getText()));
				pool.getFilterPump().setTerminate(pool.getFilterPump().stringToLocalDateTime(textField2.getText()));
				pool.setPanelIndex(3);
				currentFrame.getContentPane().removeAll();
				currentFrame.getContentPane().add(new FilterPumpModPanel(currentFrame, pool));
				currentFrame.getContentPane().revalidate();
